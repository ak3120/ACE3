...
// Defaults (good for most vehicles/binocular)
...
if (alive ACE_player) then {
    if (((vehicle ACE_player) == ACE_player) || {
        ...
        if ((cameraView == "GUNNER") && {currentWeapon ACE_player != ""} && {binocular ACE_player == currentWeapon ACE_player}) exitWith {
            ...
            if (isNumber (_config >> QGVAR(nvgGen))) then {_nvgGen = getNumber (_config >> QGVAR(nvgGen));};
        };
        ...
        if ((getText (_config >> "modelOptics")) == "") then {_borderImage = getText (_config >> QGVAR(border));};
...
GVAR(hmdBlurRadius) = _blurRadius;
GVAR(hmdGen) = _nvgGen;
systemChat format ["border %1, eye %2, hideHex %3, nvg %4, radius %5", _borderImage != "", _eyeCups, _hideHex, _nvgGen, _blurRadius];
